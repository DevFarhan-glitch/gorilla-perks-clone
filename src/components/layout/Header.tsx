...
const Header = () => {
  ...
  const mainServices = [
    ...
    { name: "Outsourced Accounting", path: "/services/outsourced-accounting" },
  ];
          ...
          {navLinks.map((link) => (
            <Fragment key={link.path}>
              ...
              {link.name === "About" && (
                <DropdownMenu>
                  <DropdownMenuTrigger
                    className={`flex items-center px-3 py-2 text-sm font-medium transition-all duration-300 hover:text-gold ${
                      isServiceActive
                        ? "text-gold"
                        : "text-muted-foreground"
                    }`}
                  ...
                  <DropdownMenuContent
                    align="start"
                    className="w-64 p-2 bg-white border-border shadow-xl rounded-xl"
                  >
